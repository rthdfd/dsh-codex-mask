 ...
 *
 * 原理：在本进程内包一层 globalThis.fetch（幂等，可多实例叠加），只对 URL 匹配到的
 * 请求做两类改写，其余流量原样直通，响应一概不改：
 *
...
export function apply(ctx, config) {
  ...
  const patterns = Array.isArray(cfg.match) ? cfg.match.filter((x) => typeof x === 'string' && x.length > 0) : []
  const log = (message) => {
    try {
  ...
  if (patterns.length === 0) {
    log('match 为空，未启用（在插件 config.match 里填目标 URL 子串）')
    return
  }
  ...
  const handler = (input, init) => {
    ...
    if (!patterns.some((pattern) => url.includes(pattern))) return undefined
    return handleMatched(input, init, cfg, persona, installationId)
  }

  const uninstall = installFetchPatch(handler)
  log(`已启用：匹配 ${patterns.join(' / ')}；UA = ${persona.ua}`)
